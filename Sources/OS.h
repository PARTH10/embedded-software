...
#define OS_MAX_USER_THREADS       15
#define OS_LOWEST_PRIORITY        15
...
typedef struct ecb
{
  uint16_t count;        // Count (when event is a semaphore)
  uint16_t waitList;     // List of threads waiting for event
} OS_ECB;
...
//   and can be between 0 and 65535.
// Output:
...
OS_ECB* OS_SemaphoreCreate(const uint16_t value);
...
OS_ERROR OS_SemaphoreWait(OS_ECB* const pEvent, const uint16_t timeout);
...
//   You cannot delete the idel thread.
//   You cannot delete a thread with prioirty lower than OS_LOWEST_PRIORITY.
...
// Valid delays range from 0 to 65535 ticks.
...
void OS_TimeDelay(const uint16_t ticks);
...
void __attribute__ ((interrupt)) SysTickISR(void);
